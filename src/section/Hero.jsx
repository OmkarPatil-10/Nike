...
const Hero = () => {
  const [bigShoeimg, setbigShoeimg] = useState(bigShoe1); // Fixed state name to match usage
    ...
    >
      <div className="relative xl:w-2/5 flex flex-col justify-center items-start w-full max-xl:padding-x pt-28">
        ...
        <h1 className="mt-10 font-palanquin text-8xl max-sm:text-[72px] max-sm:leading-[80px] font-bold">
          ...
          <br />
          <span className="text-coral-red inline-block mt-[8px]">Nike Shoes</span>
        </h1>
        ...
        <div className="flex justify-start items-start flex-wrap w-full mt-10 gap-16">
          {statistics.map((stats, index) => (
            <div key={index}> {/* Added unique key */}
              <p className="font-palanquin text-4xl font-bold">{stats.value}</p>
              <p className="font-montserrat leading-7 text-gray-700">{stats.label}</p>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="relative flex-1 flex justify-center items-center xl:min-h-screen max-xl:py-40 bg-primary bg-hero bg-cover bg-center">
        <img
          src={bigShoeimg} // Corrected state value name
          alt="shoe collection"
          width={610}
          height={500}
        ...
        <div className="flex sm:gap-6 gap-4 absolute -bottom-[10%] sm:left-[10%] max-sm:px-6">
          {shoes.map((shoe, index) => (
            <div key={index}> {/* Added unique key */}
              <ShoeCard
                imgURL={shoe}
                changeBigShoeImage={(newShoe) => setbigShoeimg(newShoe)} // Updated state setter
                bigShoeImg={bigShoeimg} // Prop name consistent with state
              />
            </div>
          ))}
        </div>
